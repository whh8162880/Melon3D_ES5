...
module rf{
    ...
    export class Drager extends MiniDispatcher{
        ...
        update(ox:number,oy:number){
            let{vStep,hStep,rect,vScroll,hScroll}=this;
            if(hStep > 0){
                ox += rect.x;
        ...
        disbind(target:RenderBase){
            ...
            target.off(MouseEventX.MouseDown,this.mouseDownHandler,this);
        }
        ...
        refreshScroll(tweener?:ITweener){
            let{hStep,vStep,rect,width,height,target, x:originx, y:originy}=this;
            let{x,y,w,h}=rect;
            if(hStep > 0){
            ...
            if(target && (hStep || vStep)){
                ...
                target.setChange(DChange.batch);
            }
        }
    ...
    export class Scroll extends Drager{
        constructor(target:RenderBase){
            ...
            let{w:width,h:height,x,y}=target;
            this.setArea(w,h,width,height,x,y);
            target.on(EventT.RESIZE,this.resizeHandler,this);
            this.target = target;
        }

        resizeHandler(event?:EventX){
            let{w:width,h:height}=event.currentTarget as {w?:number,h?:number};
            let{w,h}=this.rect; 
        ...
        get backward(){
            let{hStep, hScroll, vScroll}=this;
            let c = hStep > 0 ? hScroll.pos : vScroll.pos;
            let max = hStep > 0 ? hScroll.max : vScroll.max;
